...
/*
float Integrate_Trapeze(int finish, float h, const std::vector<float> &function)
{
	float sum = 0.f;
...
void M_calc(int k, int N, int MM, const std::vector<float> &x, const std::vector<float> &y,
			float **W, std::vector<float> &M, std::vector<float> &mst_calc1, std::vector<float> &mst_calc2,
			std::vector<float> &mst_calc3, Params params)
{
	float W_on_sqrt;
	...
	*/
	...
	for (int s = 0; s < deg_num; s++)
	{
		...
		for (int j = 0; j <= MM; j++)
		{
			for (int i = 0; i <= N; i++)
			{
				Wprev[i][j] = W_start(params.alp, x[i], y[j], r[s]);
			}
		}
		W_normirovka(N, MM, Wprev);

		find_last_W_and_M(params, params_delta1, N_Tr, N, MM, x, y, t, mst1, mst2, mst3, Wprev, r[s], Wcurr, M, mst_calc1, mst_calc2, mst_calc3, 0);
